...
import {
    ...
    Typography, Icon, Avatar, makeStyles
...
export default function EditProduct() {
    ...
    const clickSubmit = () => {
        ...
        update({ shopId, productId }, { t: jwt.token }, productData)
            .then((data) => {
    ...
    useEffect(() => {
        ...
        read({ productId }, signal).then((data) => {
            if (data.error) setValues({ ...values, error: data.error })
                ...
                setValues({
                    ...values, id: data._id, name: data.name,
    ...
    return (<div>
        <Card className={classes.card}>
            <CardContent>
                ...
                <TextField id="multiline-flexible" label="Description" multiline rows="3"
                ...
                <TextField id="category" label="Category" className={classes.textField}
                    value={values.category} onChange={handleChange('category')} margin="normal" /><br />
                <TextField id="quantity" label="Quantity" className={classes.textField} type="number"
                    value={values.quantity} onChange={handleChange('quantity')} margin="normal" /><br />
