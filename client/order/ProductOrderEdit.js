...
import {
    ...
    Typography, MenuItem, TextField, Divider
} from '@material-ui/core'

import auth from './../auth/auth-helper'
import {
    getStatusValues, update, cancelProduct,
    processCharge
} from './api-order.js'
...
const useStyles = makeStyles(theme => ({
    ...
    listDetails: { display: "inline-block" },
    listQty: {
...
export default function ProductOrderEdit(props) {
    ...
    const handleStatusChange = productIndex => event => {
        ...
        if (event.target.value == "Cancelled") {
            ...
            update({ shopId: props.shopId }, { t: jwt.token }, {
                cartItemId: product._id,
                status: event.target.value
            }).then((data) => {
                if (data.error) {
