...
const useStyles = makeStyles(theme => ({
    ...
    productTitle: {
        ...
        color: theme.palette.openTitle, width: '100%', fontSize: '1.2em'
    }
}))
...
export default function Shop() {
    ...
    useEffect(() => {
        ...
        read({ shopId }, signal).then((data) => {
            if (data.error) console.log(data.error) //setError(data.error)
            else setShop(data)
        })
            ...
            <Grid item xs={8} sm={8}>
                <Card>
                    <Typography type="title" component="h2"
                        className={classes.productTitle}>Products</Typography>
